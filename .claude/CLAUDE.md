...
`/commit` 스킬을 사용한다. 상세 동작은 `.claude/skills/commit/SKILL.md` 참고.

- **인자 있음** (`/commit <경로>`): 해당 파일/디렉토리를 바로 커밋
- **인자 없음** (`/commit`): staged 파일을 카테고리별로 분리하여 커밋 계획 제시 → 사용자 승인 후 실행

커밋 메시지 형식: `[카테고리] 설명` (Co-Authored-By Claude 자동 포함)
