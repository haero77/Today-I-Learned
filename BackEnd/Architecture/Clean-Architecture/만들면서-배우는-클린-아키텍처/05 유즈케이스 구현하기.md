...
### 입력 모델 검증과 도메인 엔티티의 자기 보호는 별개

- '이체 금액은 0보다 커야 한다'는 조건을 `SendMoneyCommand`에서 검증한다고 해서 도메인 엔티티가 언제나 호출자를 신뢰해도 된다는 뜻은 아님.
- `Money`는 잔액 변화나 손실 등 음수 금액도 표현할 수 있는 범용 값 객체이므로, `Money` 자체를 항상 양수로 제한할 필요는 없음.
- `SendMoneyCommand`의 `@PositiveMoney`는 **이체 유스케이스가 받는 입력은 양수여야 한다**는 유스케이스 입력 조건을 보장함.
- 한편 `Account.withdraw()`가 음수를 받았을 때 잔액이 오히려 증가하는 등 엔티티의 상태가 잘못될 수 있다면, 엔티티도 자신의 연산에 필요한 전제 조건을 방어해야함.
  - 이는 웹 어댑터와 입력 모델에서 같은 입력 검증을 불필요하게 반복하는 것과 다름.
  - 입력 모델의 검증은 애플리케이션 경계로 잘못된 입력이 들어오는 것을 막고, 엔티티의 검증은 다른 유스케이스나 테스트에서 직접 호출되더라도 도메인 상태가 깨지지 않도록 보호함.
...
- 중복 검증을 타입으로 제거하고 싶다면, 생성 시 양수임을 보장하는 `PositiveMoney` 값 객체를 만들고 `SendMoneyCommand`와 `Account.withdraw()`가 이 타입을 받게 할 수도 있음.
- 정리하면 `Money`의 불변 조건, 특정 유스케이스의 입력 조건, 엔티티 연산의 전제 조건을 서로 구분해야함. **도메인 객체의 올바른 상태가 특정 조건에 의존한다면 도메인 객체도 스스로 그 조건을 보장해야함.**
...
- 우리 아키텍처 스타일은 **도메인 모델을 구현하는 방법에 제약을 두지 않음.**
  - 풍부한 도메인 모델을 사용할 경우 유스케이스가 도메인 모델의 진입점 역할
  - 빈약한 도메인 모델을 사용할 경우 엔티티 자체에 도메인 로직을 포함하지 않고, 도메인 로직을 유스케이스에 구현하는 방식이 된다.

# 다양한 유스케이스에 대한 다양한 출력 모델

- 여러 유스케이스 간에 동일한 출력 모델을 사용하게 되면 해당 **유스케이스 들이 강하게 결합되는 문제 발생.**
  - 한 유스케이스의 출력 모델에 새로운 필드가 필요한 경우, 다른 유스케이스들도 **해당 필드가 자신들과 무관하더라도 처리해야함.**

# 읽기 전용 유스케이스는 어떻게 할까?

- UI에서 계좌 잔액을 표시해야한다고 가정해보자.
  - 이를 위한 전용 유스케이스를 따로 만들 것인가?
- 유스케이스로 간주할 경우, 다른 유스케이스와 동일한 방식으로 구현해야함.
- 하지만, 애플리케이션 코어 관점에서 볼 때는 **단순 데이터 쿼리에 불과.**

## 쿼리 전용 인커밍 포트와 쿼리 서비스
...
class GetAccountBalanceService implements GetAccountBalanceUseCase {
  ...
  @Override
  public AccountBalance getAccountBalance(GetAccountBalanceQuery query) {
...
- 이렇게 하면 코드베이스에서 읽기 전용 쿼리와 수정 유스케이스(커맨드)를 명확히 구분 가능. 
  - 입력 타입의 이름만 봐도 명확히 구분 가능.
- 서비스는 쿼리를 아웃고잉 포트로 전달하는 것 외에는 실제로 아무런 작업을 하지 않음. 
- 계층 간에 동일한 모델 사용 시 지름길을 선택 가능하고, 클라이언트가 아웃고잉 포트를 직접호출하게 할 수 있음.

# 이것이 유지보수 가능한 소프트웨어를 구축하는 데 어떻게 도움이 될까? 

1. 유스케이스별 모델을 사용하면 해당 유스케이스 명확하게 이해 가능 & 장기적으로 유지보수가 쉬워짐.
   1. 여러 개발자가 서로 충돌하지 않으면서 동시에 다양한 유스케이스 작업 가능.
2. 엄격한 입력 검증과 함께 유스케이스별 입/출력 모델 사용 시 유지보수 가능한 코드베이스 만드는데 도움이 됨. 
