...
* [4. 본질적인 복잡성과 우발적인 복잡성, 그리고 약간 비슷한 중복 코드를 제거하는 법](#4-본질적인-복잡성과-우발적인-복잡성-그리고-약간-비슷한-중복-코드를-제거하는-법)
  * [본질적인 복잡성과 우발적인 복잡성](#본질적인-복잡성과-우발적인-복잡성)
    * [본질적인 우발성을 내버려두지 말고 도전하자!: 비즈니스 규칙을 단순하고 직관적으로.](#본질적인-우발성을-내버려두지-말고-도전하자-비즈니스-규칙을-단순하고-직관적으로)
    * [⭐️ IF 한 줄을 쉽게 넣지 말자!! (그 'if 한 줄' 어떻게든 빼기 위해 고민하자!!)](#-if-한-줄을-쉽게-넣지-말자-그-if-한-줄-어떻게든-빼기-위해-고민하자)
...
- **본질적인 복잡성**: 
  - 문제 자체에 내재된 복잡성. 주로 비즈니스 규칙 자체가 복잡한 경우 발생.
  - 예시
    - 회원 등급에 따른 혜택 차이 
    - 특정 파트너를 위한 분기 처리 
    - 주문 상태에 따라 달라지는 동작
- 우발적인 복잡성: 
  - 문제 해결 과정에서 도구나 기술의 문제로 인해 생기는 복잡성으로 해결 가능한 복잡성
  - 코드 품질을 다루는 Simple Design은 이런 우발적 복잡성을 낮추기 위한 방법임. 
  - 조건문의 중복을 다형성으로 해결하는 것도 우발적인 복잡성을 해결하는 것임.


### 본질적인 우발성을 내버려두지 말고 도전하자!: 비즈니스 규칙을 단순하고 직관적으로.

> 개발자가 복잡하게 느껴지면 사용자도 복잡하게 느낀다!
> 
> 그러므로, 본질적인 복잡성을 낮추기 위해 규칙을 단순하고 직관적으로 만들어야한다.

- 그럼, 본질적인 복잡성을 내버려둬야하나? 
  - No. 본질적인 복잡성에도 도전하자. 비록 우리가 기획자가 아니더라도.  
  - 개발자가 (본질적인 복잡성으로 인해) 복잡하게 느낀다면, 사용자에게는 더 복잡하게 느껴진다!
    - 조건이 복잡하고 많다는 것은, 그것을 쓰는 사용자도 여러가지 케이스를 머리 속에 담고 써야한다는 뜻 (인지 부하)
- **본질적 복잡성이든, 우발적 복잡성이든 코드 품질이 미치는 악영향은 동일!**
  - 설령 다형성으로 풀어내도, 다형성의 구현은 가볍지 않다.
-  👉  **_코드 품질을 떨어뜨리는 비즈니스 규칙이라면 정말로 비즈니스 가치가 높은 규칙인지 의심해볼 필요가 있다._** ⭐
    - **_규칙이 단순하고 직관적이어야 사용자가 의도대로 제품을 사용할 가능성이 높아진다._** ⭐⭐
- *비즈니스 사이드에 있는 사람들이나 기획자는 복잡성이 크게 느껴지지 않을 수도 있다. (*단건 단건 기획을 하기 때문에)
  - but, 개발자는 이전에 쌓여있던 기획이 코드에 녹아든 상태이기 때문에 이 복잡성을 더 실감나게 느낄 수 있다.
  - 어떻게 보면 조직에서 비즈니스 복잡성을 제일 실감나게 느끼는 사람은 개발자임.
    - 👉 **_그래서, 개발자가 이 복잡성을 비즈니스 사이드에 알려줄 의무가 있다고 생각함!!_**
      - "_우리가 이렇게 복잡하니까 사용자도 복잡하게 느낄 것이다. 더 단순화해보자~_" 이런 제안을 해볼 수 있음.
      - 비즈니스 복잡성이 크면 좋은 코드를 잘 작성하는 팀도 유지보수 불능 상태에 빠지는 경우가 많다. 그러니까, 본질적인 복잡성도 낮추기 위해 노력해야함.

### ⭐️ IF 한 줄을 쉽게 넣지 말자!! (그 'if 한 줄' 어떻게든 빼기 위해 고민하자!!)

- 앞선 얘기들에서 내가 전달하고 싶은게 이거였음.
- 어떤 기능 요청을 받을 때, if 한 줄만 넣으면 되는거 아니야? 싶은 경우가 많음.
  - 이런 'if 한 줄'를 쉽게 넣으면 안된다.
- 👉 **_그 'if 한 줄' 어떻게든 빼기 위해 고민하면, 중복 코드도 줄어들고, 결국 더 단순한 코드를 만들 수 있다!_** ⭐⭐
