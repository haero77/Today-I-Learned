> ### [JPA] 페이징과 컬렉션 엔티티를 함께 사용하기 (feat. @BatchSize)
...
컬렉션 엔티티 조회 시 발생하는 N+1 문제를 해결하기 위해, `@OneToMany`로 매핑된 컬렉션 엔티티 필드에 `org.hibernate.annotations.BatchSize` 어노테이션을 작성하고 size는 100으로 지정했다. `@BatchSize`를 사용하면, 연관된 엔티티 조회 시 지정한 size 만큼 `IN` 쿼리 사용하여 조회한다.
...
![](https://velog.velcdn.com/images/balparang/post/d6691c74-186d-46ac-8970-9a550a080812/image.png)

_(@BatchSize 적용 전: 1+N 쿼리 실행)_

![](https://velog.velcdn.com/images/balparang/post/8124ae50-c9bb-4aab-a0c2-65202775df55/image.png)

_(@BatchSize 적용 후: 1+1 쿼리 실행)_

N+1 문제를 해결한 후 해당 로직을 사용하는 API 응답속도를 측정해보니 0.71초에서 0.12초로 줄은 것을 확인할 수 있었다. (더욱 상세한 성능 테스트는 추후 포스팅으로 다룰 예정이다.)
